...
public class OBD2CoreUtils {
	...
	public static boolean isELM327SupportedPIDObj(JSONObject pidObj){
		return pidObj.has(OBD2CoreConstants.CONF_MODE) && pidObj.has(OBD2CoreConstants.CONF_PID);
	}

	public static boolean isVendorSupportedPIDObj(JSONObject pidObj, String vendor) {
		try {
			return pidObj.has(OBD2CoreConstants.CONF_EX_PLATFORMS) &&
					pidObj.getJSONObject(OBD2CoreConstants.CONF_EX_PLATFORMS).has(vendor);
